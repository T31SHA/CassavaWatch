...
with sync_playwright() as p:
    ...
    small = page.evaluate("""[...document.querySelectorAll('button:not([hidden]), .bigbtn, select')]
        .filter(e => e.offsetParent && !e.closest('#thumbs') && e.getBoundingClientRect().height < 48)
    ...
    check("capture: step 3 result shown", page.locator(".steps li.on").inner_text().endswith("Result"))
    ...
    fill_photos(page, IMGS)
    page.click("#submit")
    expect(page.locator("#msg")).to_contain_text("saved on this phone")
    expect(page.locator("#net")).to_have_text("Offline · 1 queued")
    check("offline: report queued + pill shows count", True)
    n_q = len(api("/api/reports?days=1"))
    ctx.set_offline(False)
    expect(page.locator("#msg")).to_contain_text("Queued reports sent: 1", timeout=20000)
    ...
    page.set_input_files("#photo", str(bad))
    page.click("#submit")
    ...
    expect(page.locator("#alerts tbody tr")).to_have_count(1)
    page.click("#run")
    expect(page.locator("#flash")).to_contain_text("Surveillance run")
    row = page.locator("#alerts tbody tr").first
    check("dashboard: injected CBSD alert visible", "CBSD" in row.inner_text(), row.inner_text().replace("\n", " "))
    page.locator("#f-days").select_option("7")
    expect(page.locator("#f-count")).not_to_have_text("")
    row.locator("td").nth(1).click()
    page.wait_for_timeout(800)
    zoom = page.evaluate("map.getZoom()")
    check("dashboard: row click zooms to cell", zoom >= 10, f"zoom={zoom}")
    page.locator("#f-days").select_option("60")
    page.locator("#f-disease").select_option("all")
    page.evaluate("fitted=false; map.setView([-1.0,33.5],7)")
    page.wait_for_timeout(1500)
    page.screenshot(path=SHOTS / "dashboard.png")
    page.locator("#f-disease").select_option("cgm")
    page.locator("#f-days").select_option("7")
    ...
    page.goto(BASE + "/dashboard")
    page.wait_for_timeout(800)
    ya = page.locator(".alerts-panel").bounding_box()["y"]
    ym = page.locator(".map-panel").bounding_box()["y"]
